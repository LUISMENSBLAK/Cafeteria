...
export type PrintOrderData = EscPosOrderData
...
async function printWithNetwork(orderData: PrintOrderData, settings: PrinterSettings): Promise<PrintResult> {
  ...
  try {
    ...
    for (const item of orderData.items) {
      ...
      if (item.extra_nombre && item.extra_precio != null) xml += `<text>  + ${escapeXml(item.extra_nombre)} $${(item.extra_precio * item.cantidad).toFixed(2)}\n</text>`
    }
  ...
  return `
    <div style="font-family: Arial, sans-serif; color:#111; font-size:${fontSize}; width:${widthMm}; padding:3mm; box-sizing:border-box; background:white;">
      ...
      ${orderData.items.map(item => `
        <div style="display:flex; justify-content:space-between; gap:8px; margin-bottom:4px;"><div>${item.cantidad}x ${escapeHtml(item.nombre)}</div><div>$${(item.precio_unitario * item.cantidad).toFixed(2)}</div></div>
        ${item.variante_nombre ? `<div style="padding-left:12px; font-size:10px;">- ${escapeHtml(item.variante_nombre)}</div>` : ''}
        ${(item.extras_pago ?? []).map(extra => `<div style="display:flex; justify-content:space-between; padding-left:12px; font-size:10px;"><div>+ ${escapeHtml(extra.nombre)}</div><div>$${(extra.precio * item.cantidad).toFixed(2)}</div></div>`).join('')}
        ${item.extra_nombre && item.extra_precio != null ? `<div style="display:flex; justify-content:space-between; padding-left:12px; font-size:10px;"><div>+ ${escapeHtml(item.extra_nombre)}</div><div>$${(item.extra_precio * item.cantidad).toFixed(2)}</div></div>` : ''}
...
function printWithBrowser(orderData: PrintOrderData, settings: PrinterSettings): PrintResult {
  ...
  if (!printArea) {
    ...
    printArea.id = 'abaroa-print-area'
    document.body.appendChild(printArea)
    const style = document.createElement('style')
    style.textContent = `
      @media print {
        body > *:not(#abaroa-print-area) { display:none !important; }
        body { margin:0 !important; padding:0 !important; background:white !important; }
        #abaroa-print-area { display:block !important; width:${widthMm}; margin:0; padding:0; }
        @page { margin:0; size:${widthMm} auto; }
      }
      @media screen { #abaroa-print-area { display:none !important; } }
    `
